 ...
 */
	...
	void RobotInit() {
		...
		leftFront->ConfigPeakOutputForward(1, kTimeoutMs);
		leftFront->ConfigPeakOutputReverse(-1, kTimeoutMs);
		...
		rghtFront->ConfigPeakOutputForward(1, kTimeoutMs);
		rghtFront->ConfigPeakOutputReverse(-1, kTimeoutMs);
	...
	void Drive(float dist) {
			leftFront->Set(ControlMode::Position, -dist);
			rghtFront->Set(ControlMode::Position, dist);
	}
	void turnRight(float dist)
	{
		resetEncoders();
		while (rghtFront->GetSelectedSensorPosition(0) > dist*-1 )
		{
			leftFront->Set(ControlMode::Position, -dist);
			rghtFront->Set(ControlMode::Position, -dist);
		}
...
//		//decides the behavior of the motors when power is set to 0 (other option is coast)
	...
	void AutonomousPeriodic() {
		...
		rotLeft = leftFront->GetSelectedSensorPosition(0);
		...
		else if (m_autoSelected == left)
		{
			...
			{ //it's gonna move forward, turn, and put a cube in the switch
				...
				else if (rotLeft >= 6.00 && rotLeft <= 7.38)
				{//90 degree turn
					_diffDrive->ArcadeDrive(0.0,-0.5);
